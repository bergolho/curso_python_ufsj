...
\subtitle{IV Congresso de Administração e Tecnologia - Grambery - 2018}
\institute{}
\date{}
...
\item Você simplesmente executa o comando python e pode começar a executar código de forma	interativa
  ...
  \begin{python}
    ...
    [GCC 8.1.1 20180712 (Red Hat 8.1.1-1)] on linux2
...
\item ou através da linha de comando (mostrar no Spyder):
  
  \item \$ python3 programa.py 
  \ei
  \ei
\end{frame}
...
\begin{frame}[fragile]
  ...
  \item Vamos apresentar de forma prática alguns destes mecanismos:
    ...
    \item \pythoninline{map} e \pythoninline{filter}
    \ei
...
[10, 11, 12, 13, 14]
>>> print(list(map(sum, range(5), range(10,15)))) 
[10, 12, 14, 16, 18]    
  \end{python}
\end{frame}
...
44 48    
  \end{python}
  \bi
  \item O uso de funções lambda com map e filter
    é muito prático
  \ei
  \begin{python}
...
\begin{frame}[fragile]
	...
	\ei
	\begin{python}
with open("dados.txt") as infile:
	read_data = infile.read()
	print(read_data)
	\end{python}
	\bi
	\item Conferindo se o arquivo esta aberto ou fechado
	\ei
	\begin{python}
...
\begin{frame}[fragile]
	\ft{Arquivos}
	\bi
	\item Lendo um arquivo linha por linha:
	\ei
	\begin{python}
with open("pontos.txt") as infile:
	...
	\end{python}
	\bi
	\item Podemos quebrar a linha em várias partes
	\ei
	\begin{python}
# linhas no formato: x y z
temp = line.split()
...
\begin{frame}[fragile]
  \ft{\textbf{Hands on}}
  \bi
  \item[2.] Vamos implementar uma função \pythoninline{le\_pontos()} que recebe a string
...
\begin{frame}[t]{Agradecimentos}
	\bi
	\item Profa. Tatiane Ornelas Martins Alves
	\item Prof. Rafael Sachetto Oliveira - UFSJ
	\item Prof. Bernardo Martins Rocha - UFJF
	\item Departamento de Ciência da Computação - UFSJ
	\item Departamento de Ciência da Computação - UFJF
	\item Departamento de Sistemas de Informação - Grambery
	\ei
\end{frame}
